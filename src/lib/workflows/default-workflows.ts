...
Follow-up: Check in [X] days if no tracking received.`,
  ...
  // ─── 2. Replacement Order ─────────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['replacement', 'send me a new one', 'replace', 'damaged in shipping', 'wrong item sent', 'defective', 'does not match the size', 'sizes do not match', 'one is bigger than the other', 'different size instead', 'my approved replacement'],
    whenToUse: [
...
Next: Follow up in [X] days for shipping confirmation.`,
  ...
  // ─── 3. Damaged / Defective Item ──────────────────────────────────────────
  {
    ...
    triggerPhrases: ['damaged', 'defective', 'broken', 'torn', 'ripped', 'falling apart', 'quality issue', 'not as expected', 'peeling', 'peeling off', 'falling off', 'number is falling off', 'number is peeling', 'reflective dots', 'dots are damaged', 'same defect', 'defect', 'design is wrong again', 'wore it once', 'ripping at the seam', 'after a few wears', 'after a couple of wears'],
    whenToUse: [
...
Next: Await photos / Resolution pending.`,
  ...
  // ─── 4. Wrong Item Received ───────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['wrong item', 'wrong product', 'received wrong', 'not what I ordered', 'got the wrong thing', 'sent wrong', 'you sent me', 'sent me a', 'you sent', 'you put', 'put a red', 'shipped instead', 'wrong club', 'different club', 'sponsor logos', 'belong to a different club', 'sent the wrong', 'wrong color', 'no logo', 'no design', 'plain white', 'blank jersey', 'correct shirt'],
...
Action: [Pending photos / Replacement ordered]`,
  ...
  // ─── 5. Missing Item ──────────────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['missing item', 'item not in box', 'did not receive', 'not in my order', 'incomplete order', 'only got part of my order', 'is missing', 'was not in the package', 'not in the package', 'only received', 'forgot to include', 'one item missing'],
...
Action: Provided processing timeline to customer.`,
  ...
  // ─── 10. Order Change ─────────────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['change my order', 'update my order', 'wrong address', 'wrong size ordered', 'change shipping address', 'modify order', 'change', 'change the size', 'change all the sizes', 'change my size', 'too late to change', 'is it too late', 'want to change'],
...
Action: Checking internally. Customer informed of timeline.`,
  ...
  // ─── 11. Order Cancellation ───────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['cancel my order', 'cancel order', 'I want to cancel', 'please cancel', 'stop my order', 'cancel', 'cancel it', 'cancel one', 'want to cancel it', 'entered by mistake', 'placed by mistake', 'ordered by mistake'],
...
Action: Provided wave status update to customer.`,
  ...
  // ─── 13. Team Store Password ──────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['team store password', 'team shop password', 'store password', 'cannot access store', 'password for team store', 'club store access', 'password', 'team store', 'access the store', 'get into the team store', 'log into the store'],
...
Note: Password shared internally only, not via external channel.`,
  ...
  // ─── 14. Player Link ──────────────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['player link', 'player page', 'individual link', 'personal link', 'can I order individually', 'player store', 'player number', 'player roster', 'roster', 'which team package', 'my own link', 'need my link', 'number changed', 'still shows the old number'],
...
Status: On-hold/Pending — awaiting Team Leader.`,
  ...
  // ─── 17. Out of Stock ──────────────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['out of stock', 'not available', 'sold out', 'when will it be back', 'back in stock', 'unavailable', 'restock', 'when will this be available', 'available to purchase', 'available for sale', 'pre-order', 'preorder', 'pre order', 'replica jersey'],
...
Action: Redirected to official CS channel. No internal contact information shared.`,
  ...
  // ─── 19. Tracking Not Moving ──────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['tracking not updating', 'no movement', 'tracking stuck', 'says shipped but no update', 'package not moving', 'tracking', 'label created', 'label was created', 'not been picked up', 'not picked up', 'no tracking information', 'no information', 'has no information', 'stuck on label', 'shipped but no tracking'],
...
Action: [Provided order info / Guided account creation]`,
  ...
  // ─── 21. Individual Item Ordering ─────────────────────────────────────────
  {
    ...
    triggerPhrases: ['order one jersey', 'buy individual', 'single item', 'purchase just one', 'order without team', 'not in a club', 'individual item', 'smaller items', 'a few smaller items', 'enable adding', 'order just one', "don't want to buy the entire", 'without the entire kit', 'add individual'],
    whenToUse: [
...
Action: [Provided info / Escalated for missing info]`,
  ...
  // ─── 25. Website Issue ────────────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['website not working', 'cannot checkout', "can't add to cart", 'site error', 'page broken', 'not loading', 'error message', 'website', "can't access my cart", 'cannot access my cart', 'complete my order', "can't complete", 'cart', "won't let me", 'glitch'],
    whenToUse: [
...
Escalated to tech team: [YES/NO]`,
  ...
  // ─── 26. Expedited Shipping ───────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['rush order', 'expedited shipping', 'need it faster', 'upgrade shipping', 'overnight', '2-day shipping', 'need by date', 'expedite', 'rush', 'need it sooner', 'need it by', 'in time for', 'registered late', 'late registration', 'tournament this weekend', 'before the game'],
    whenToUse: [
...
Action: Provided tracking info to customer.`,
  ...
  // ─── 28. Partial Shipment ─────────────────────────────────────────────────
  {
    ...
    triggerPhrases: ['only received part', 'partial shipment', 'missing part of order', 'split shipment', 'some items missing', 'got some items', 'rest of my order', 'rest of the order', 'others got theirs', 'teammates got theirs', 'which pieces', 'still waiting on the rest', 'held up'],
    whenToUse: [
